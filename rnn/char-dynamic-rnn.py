...
def build_basic_rnn_graph_with_list(
    ...
    # Create dynamic RNN with outputs [BATCH_SIZE x SEQ_LENGTH x HIDDEN_SIZE]
    ...
    rnn_outputs_unstacked = tf.unstack(rnn_outputs, axis=1)
    print("rnn_outputs_unstacked =", len(rnn_outputs_unstacked),  " of shape",  rnn_outputs_unstacked[0].shape)
    ...
    logits = [tf.matmul(rnn_output, W) + b for rnn_output in rnn_outputs_unstacked]
    print("logits =", len(logits),  " of shape",  logits[0].shape)
...
LOG_DIR = '/tmp/tf/dynamic_char_rnn/'
# Text file.
